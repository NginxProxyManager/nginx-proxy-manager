...
async function readLastLines({ filePath, maxLines, search, since, type }) {
	...
	const MAX_SCAN_BYTES = 10 * 1024 * 1024; // 10MB safety bound
	const lines = [];
	let fileHandle = null;
	...
	try {
		...
		// Never scan further back than the budget allows, however large the file is
		const minPosition = Math.max(0, fileSize - MAX_SCAN_BYTES);

		let position = fileSize;
		const buffer = Buffer.alloc(chunkSize);
		let leftover = "";
		// Orphaned UTF-8 continuation bytes carried over from the head of the chunk we
		// read previously (i.e. the one immediately after this one in the file).
		let leftoverBytes = Buffer.alloc(0);

		const sinceTime = since ? moment(since, moment.ISO_8601, true) : null;
		while (position > minPosition && lines.length < maxLines) {
			const readSize = Math.min(chunkSize, position - minPosition);
			position -= readSize;

			const { bytesRead } = await fileHandle.read(buffer, 0, readSize, position);

			// Reading backwards lands on arbitrary byte offsets, so a chunk can begin in the
			// middle of a multibyte character. Strip those leading continuation bytes and hand
			// them to the next (earlier) chunk, which holds their leading byte, instead of
			// letting them decode to U+FFFD. A character is at most 4 bytes, so at most 3
			// continuation bytes can be orphaned. Only done when an earlier chunk will actually
			// be read: at the start of the file (or of the scan budget) the bytes are genuine.
			let start = 0;
			if (position > minPosition && position > 0) {
				while (start < bytesRead && start < 3 && (buffer[start] & 0xc0) === 0x80) {
					start++;
				}
			}

			const chunkStr =
				Buffer.concat([buffer.subarray(start, bytesRead), leftoverBytes]).toString("utf8") + leftover;
			leftoverBytes = start > 0 ? Buffer.from(buffer.subarray(0, start)) : Buffer.alloc(0);
		...
		// When the scan stopped at the budget instead, the leftover is a truncated line
		// from the middle of the file and is discarded.
		if (leftover && position === 0 && lines.length < maxLines) {
			if (sinceTime) {
...
router
	...
	.get(async (req, res, next) => {
		try {
			if (req.query.lines !== undefined && (typeof req.query.lines !== "string" || !/^\d+$/.test(req.query.lines))) {
				throw new errs.ValidationError("lines must be an integer");
			}
			...
			if (data.since && !moment(data.since, moment.ISO_8601, true).isValid()) {
...
router
	...
	.get(async (req, res, next) => {
		try {
			...
			// Null-prototype maps so log-derived keys ("constructor", "__proto__", ...)
			// cannot collide with Object.prototype members.
			const status_codes = Object.create(null);
