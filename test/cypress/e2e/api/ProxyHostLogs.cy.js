...
describe('Proxy Host Logs endpoints', () => {
	...
	it('Should return 403 for unauthenticated request', () => {
		cy.task('backendApiGet', {
			path:  `/api/nginx/proxy-hosts/${hostId}/logs`,
			returnOnError: true
		}).then((data) => {
			expect(data).to.have.property('error');
			// jwtdecode/access.can returns 403 Permission Denied when no token is provided
			expect(data.error.code).to.be.equal(403);
		});
	});
	...
	describe('With populated log files', () => {
		...
		before(() => {
			// Populate the access logs, then the error logs
