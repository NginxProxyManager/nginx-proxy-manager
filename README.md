...
**Runner:** `runs-on: [self-hosted, linux, gen]` — internal Infisical vault; Docker Buildx (`docker/build-push-action`) on the runner host via `DOCKER_HOST` / `/var/run/docker.sock`. Frontend is built with Node/yarn (not `docker run`). Image platform: `linux/amd64`. No GitHub repository variables or secrets are required.
