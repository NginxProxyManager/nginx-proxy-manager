...
**Runner:** `runs-on: [self-hosted, linux, gen]` — internal Infisical vault; image build uses `docker build` on the host engine via `DOCKER_HOST` (not Buildx/nested containers). Frontend is built with Node/yarn. Image platform: `linux/amd64`. No GitHub repository variables or secrets are required.
