...
For that to work, the NPM admin API must be reachable on the same host (for example reverse-proxy `/api` to port `81`). See [deploy/nginx/docs-api-proxy.conf.example](../../../deploy/nginx/docs-api-proxy.conf.example) in the repository for a sample nginx config. If you only host static docs without `/api`, use [Redoc](/api-reference/) for reading and the [Automation API](/advanced/automation-api) guide for curl examples.
