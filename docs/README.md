...
Bundled Swagger uses server URL `/api` on the **same origin** as the docs. To host static docs on another domain and still try requests, reverse-proxy `/api` to the NPM admin API (port 81). Sample config: `docker/docs-api-proxy.conf.example` in the repository.
...
See [VERSIONING.md](VERSIONING.md) and [SECURITY.md](../SECURITY.md).
