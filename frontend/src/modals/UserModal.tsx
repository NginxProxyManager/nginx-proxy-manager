...
const UserModal = EasyModal.create(({ id, visible, remove }: Props) => {
	...
	const isSelf = id !== "new" && !!data && data.id === currentUser?.id;
	...
	return (
		<Modal show={visible} onHide={remove}>
			<Modal.Header closeButton>
				<Modal.Title>
					<T
						id={isSelf ? "user.edit-profile" : id === "new" ? "object.add" : "object.edit"}
						tData={{ object: "user" }}
					/>
				</Modal.Title>
			</Modal.Header>
			{isSelf && (
				<div
					className="nav nav-tabs px-3"
					role="tablist"
					aria-label={intl.formatMessage({ id: "user.edit-profile" })}
				>
					<button
						type="button"
						role="tab"
						aria-selected={section === "profile"}
						className={`nav-link${section === "profile" ? " active" : ""}`}
					...
					<button
						type="button"
						role="tab"
						aria-selected={section === "login"}
						className={`nav-link${section === "login" ? " active" : ""}`}
						onClick={() => setSection("login")}
						disabled={isSubmitting}
					>
						<T id="oidc.login-methods" />
					</button>
				</div>
			)}
			{isSelf && section === "login" && (
				<Modal.Body>
					<LoginMethods />
				</Modal.Body>
			)}
			...
			{!isLoading && !currentIsLoading && data && currentUser && (
				<div hidden={isSelf && section !== "profile"}>
					<Formik
						initialValues={
							{
								name: data?.name,
