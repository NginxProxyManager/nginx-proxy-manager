...
Сертификат, подтверждённый по HTTP, означает, что серверы Certbot попытаются обратиться к вашим доменам по HTTP (не HTTPS!) и при успехе выпустят сертификат.
...
Сертификат, подтверждённый по DNS, требует использования плагина DNS‑провайдера. Такой провайдер создаст временные записи в вашем домене, затем Certbot проверит эти записи, чтобы убедиться, что вы владелец домена, и при успехе выпустит сертификат.
...
Используйте этот вариант, чтобы загрузить собственный TLS‑сертификат, выданный вашим удостоверяющим центром (CA).
